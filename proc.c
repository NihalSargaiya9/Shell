...
void exit(int status)
{
  ...
  struct proc *p;
  int fd;
  myproc()->exitStatus = status; // seting value of status variable
  if (curproc == initproc)
    panic("init exiting");
...
void procinfo(int pid)
{
  ...
  for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)
  {
    if (p->pid == pid)
    {
      ...
      cprintf("Last Context Switched out time: %d:%d:%d %d:%d:%d", lastContextInDT.second, lastContextInDT.minute, lastContextInDT.hour, lastContextInDT.day, lastContextInDT.month, lastContextInDT.year);
